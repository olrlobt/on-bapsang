...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT new com.on_bapsang.backend.dto.PostSummary(" +
            "p.id, p.title, p.imageUrl, p.scrapCount, p.commentCount, p.createdAt, p.user.nickname, p.x, p.y) " +
            "FROM Post p")
    ...
    @Query("SELECT new com.on_bapsang.backend.dto.mypage.ScrappedPost(" +
            "p.id, p.title, p.imageUrl, p.scrapCount, p.commentCount, p.createdAt, p.x, p.y) " +
